...
const UserEdit = () => {
    ...
    const onSubmit = async (values: z.infer<typeof SettingsSchema>) => {
        ...
        startTransition(() => {
            ...
            const uploadImagePromise = values.image
                ? new Promise((resolve, reject) => {
                        const formData = new FormData();
                        formData.append('file', values.image);
                        formData.append('upload_preset', 'your-upload-preset'); // Replace 'your-upload-preset' with your actual upload preset name
                        ...
                        fetch(`https://api.cloudinary.com/v1_1/your-cloud-name/image/upload`, {
                            ...
                            .then((data) => {
                                // Resolve with the Cloudinary URL of the uploaded image
                ...
                : Promise.resolve(undefined); // Resolve immediately if no image is provided
    
        // Wait for the image upload promise to resolve
        uploadImagePromise
            .then((imageUrl) => {
                // Proceed with other form submission logic (e.g., updating user settings)
                settings({ ...values, image: imageUrl }) // Pass the Cloudinary URL of the uploaded image to the settings function
                    .then((data) => {
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                {field.value && (
                                                    <AdvancedImage cldImg={cld.image(field.value).resize(fill().width(100).height(150))} />
                                                )}
                                                <FormControl>
