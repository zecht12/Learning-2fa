...
const UserEdit = () => {
    ...
    return (
        <div className={`bg-[radial-gradient(ellipse_at_top_var(--tw-gradient-stops))] ${user?.role === "USER" && user?.isOAuth === true && "w-full h-screen"} ${user?.role === "USER" && user?.isOAuth === false && "w-full h-auto"} ${user?.role === "ADMIN" && user?.isOAuth === true && "w-full h-screen"} ${user?.role === "ADMIN" && user?.isOAuth === false && "w-full h-auto"}from-slate-800 to-zinc-900 py-10 flex items-center justify-center`}>
