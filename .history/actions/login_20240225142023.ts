...
import { DEFAULT_LOGIN_ADMIN_REDIRECT, DEFAULT_LOGIN_REDIRECT } from '@/routes';
...
export const login = async (values : z.infer<typeof LoginSchema>) => {
    ...
    const redirectTo = await useCurrentRole();
    if (condition) {
    ...
    try {
        await signIn("credentials", { 
            email,
            password,
            redirectTo : DEFAULT_LOGIN_REDIRECT  || DEFAULT_LOGIN_ADMIN_REDIRECT
        });
    } catch (error) {
